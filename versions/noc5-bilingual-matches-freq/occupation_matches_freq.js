// Qualtrics occupation widget — MATCH FEEDBACK + CENSUS FREQUENCY ORDERING
// Depends on: noc2021_bilingual_matches_freq.js (defines window.nocMatchesFreq)
//
// Identical to noc5-bilingual-matches, except that results are ordered by how
// many people actually hold each occupation, from the 2021 Census, so the
// commonest categories surface first. Set tierFirst: true to put whole-phrase
// title matches above the frequency ordering instead.
//
...
(function () {
    ...
    //     window.occupationWidget.matchesFreq.init(this, { fieldPrefix: "" });
    //   });
    //
    ...
    window.occupationWidget.matchesFreq = { init: function (engine, options) {
    ...
    var SESSION_KEY = FIELD_PREFIX + "occupation_matchesfreq_code";
    ...
    // false: order purely by census frequency, commonest first.
    // true : rank whole-phrase matches above the rest, frequency within each tier.
    var TIER_FIRST     = options.tierFirst === true;
    ...
    function waitForDeps(fn) {
        ...
        (function poll() {
            if (window.nocMatchesFreq && window.nocMatchesFreq.length && typeof TomSelect !== "undefined") { fn(); return; }
            if (Date.now() - started >= DEPS_TIMEOUT_MS) {
                console.error("[occupation-matches-freq] dependencies never loaded after " + (DEPS_TIMEOUT_MS / 1000) +
                    "s. Missing: " +
                    [!(window.nocMatchesFreq) ? "window.nocMatchesFreq (add this version's data file to Look & Feel -> Header)" : null,
    ...
    waitForDeps(function () {
        ...
        var titleIndex = [];
        window.nocMatchesFreq.forEach(function (cat) {
        ...
        // ── Census frequency ──────────────────────────────────────────────────
        // Workers per NOC category, 2021 Census. Used to order results so the
        // commonest occupations surface first. Five senior-manager categories
        // (00011-00015) are collapsed into an aggregate in the census file and
        // therefore carry 0, which sorts them last.

        var freqByCode = {};
        window.nocMatchesFreq.forEach(function (cat) {
            freqByCode[cat.category_code] = cat.freq || 0;
        });

        // ── Option builder (unchanged from tomselect) ─────────────────────────

        function buildOptions(lang) {
            return window.nocMatchesFreq.map(function (cat) {
                return {
        ...
        if (!selectEl) {
            console.error("[occupation-matches-freq] <select> not found in question " + questionId);
            return;
        }
        ...
        var control = new TomSelect(selectEl, {
            ...
            score: function (search) {
                ...
                return function (item) {
                    ...
                    // +1 so a category with no census count still scores above
                    // zero; a zero here would drop it from the results entirely.
                    var freq = (freqByCode[item.code] || 0) + 1;
                    if (!phrase || !TIER_FIRST) return freq;

                    // Tier dominates, frequency orders within it. The multiplier
                    // exceeds the largest census count (about 632,000), so no
                    // frequency can promote an item past a higher tier.
                    return tierFor(item.code, phrase, tokens) * 10000000 + freq;
                };
            },
            render: SHOW_HINTS ? {
